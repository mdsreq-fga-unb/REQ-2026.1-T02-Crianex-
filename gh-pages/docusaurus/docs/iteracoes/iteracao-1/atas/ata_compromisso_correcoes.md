...
- **Uso Enxuto do Kanban:** O Kanban no GitHub Projects será utilizado estritamente para o controle do WIP (_Work in Progress_). As colunas serão configuradas como: `Backlog` → `Design` → `Build` → `Review` → `Done`.
...
Foi identificado que o integrante **Philipe** não está conseguindo ser marcado nas issues e aparenta estar sem autorização de escrita no repositório principal (gerido pelo professor).

- **Ação:** Philipe deve realizar um pequeno commit/push de teste. Caso persista o bloqueio, o PM intercederá com o professor. As tarefas de Philipe serão passadas via caderno/Discord provisoriamente.
