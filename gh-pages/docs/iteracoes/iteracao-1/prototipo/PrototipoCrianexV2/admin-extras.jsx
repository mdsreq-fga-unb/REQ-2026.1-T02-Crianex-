      ...
      footer={
        <>
          ...
          <button className="btn sm" onClick={save}>
            Salvar alterações <Icon name="check" size={14} />
          </button>
        </>
      }
    >
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>
              Última alteração há 38 dias
            </div>
          </div>
          <button className="btn ghost sm" onClick={() => setPwOpen(!pwOpen)}>
            Alterar senha
          </button>
        </div>
        {pwOpen && (
...
const AdminFinance = () => {
  ...
  const series = useMemoE(() => {
    const gen = (b, v) =>
      Array.from({ length: 32 }, (_, i) =>
        Math.round(b + Math.sin(i / 3 + Math.random()) * v + Math.random() * v * 0.3)
      );
  ...
  return (
    <>
      ...
      <div className="admin-content">
        ...
        <div className="kpi-grid">
          ...
          <div className="kpi">
            <div className="label">
              <span
                style={{ width: 6, height: 6, borderRadius: 50, background: 'var(--purple)' }}
              />
              Em aberto
            </div>
            <div className="value">{fmtBRL(totals.pending)}</div>
            <div className="delta" style={{ color: 'var(--text-muted)' }}>
              {filtered.filter((t) => t.status === 'pending').length} faturas
            </div>
          </div>
          <div className="kpi">
            <div className="label">
            ...
            <div className="value" style={{ color: 'var(--pink)' }}>
              {fmtBRL(totals.overdue)}
            </div>
            <div className="delta down">
              {filtered.filter((t) => t.status === 'overdue').length} faturas
            </div>
          </div>
          <div className="kpi">
            <div className="label">MRR projetado</div>
            <div className="value">
              R$ 312<span className="unit">k</span>
            </div>
            <div className="delta up">
              <Icon name="arrowUp" size={11} />
              +12.4%
        ...
        <div className="filter-bar">
          ...
          <div className="filter-chip">
            ...
            <select value={productF} onChange={(e) => setProductF(e.target.value)}>
              ...
              {['Avali', 'Pontua', 'Notifly', 'Trilho', 'Atende', 'Ledger'].map((p) => (
        ...
        <div className="panel">
          <div className="panel-head">
            ...
            <span className="mono" style={{ fontSize: 11, color: 'var(--text-faint)' }}>
              {fmtBRL(totals.paid)} no total
            </span>
          </div>
          <div className="chart">
            <AreaChart series={series} color="#66df7a" />
          </div>
        </div>
        ...
        <div className="panel">
          ...
          <div className="data-table">
            <div
              className="dt-row header"
              style={{ gridTemplateColumns: '100px 90px 1.2fr 1fr 130px 110px 40px' }}
            >
              <span>Fatura</span>
              <span>Data</span>
              <span>Cliente</span>
          ...
          >
            <span className="mono" style={{ fontSize: 11, color: 'var(--text-muted)' }}>
              página 1 · 12 de {filtered.length}
            </span>
            <div style={{ display: 'flex', gap: 6 }}>
              <button className="btn sm ghost" style={{ padding: '5px 10px' }}>
                ←
              </button>
              <button className="btn sm ghost" style={{ padding: '5px 10px' }}>
                →
      ...
      {/* Report modal */}
      {reportOpen && (
        <FinanceReport totals={totals} period={period} onClose={() => setReportOpen(false)} />
      )}
    </>
  );
};
  ...
  >
    <div style={{ display: 'flex', gap: 24, marginBottom: 24, alignItems: 'baseline' }}>
      <div>
        ...
        <div style={{ fontSize: 32, letterSpacing: '-0.025em', fontWeight: 500, marginTop: 4 }}>
          {fmtBRL(tx.value)}
        </div>
      </div>
      <span className={`status-pill ${tx.status}`}>
        <span className="dt" />
        {tx.status === 'paid' ? 'pago' : tx.status === 'pending' ? 'em aberto' : 'em atraso'}
      </span>
    </div>
    ...
    <div className="data-table" style={{ marginBottom: 18 }}>
      {[
        ...
        <div key={k} className="dt-row" style={{ gridTemplateColumns: '160px 1fr' }}>
          <span
            className="mono"
            style={{ color: 'var(--text-muted)', fontSize: 11.5, letterSpacing: '0.04em' }}
          >
            {k}
          </span>
      ...
      <div className="timeline" style={{ marginTop: 8 }}>
        <div className="timeline-item call">
          ...
          <div>
            ...
            <div className="body">
              <span className="actor">Webhook Stripe</span> · pagamento confirmado e conciliado.
            </div>
          </div>
        </div>
        <div className="timeline-item">
          ...
          <div>
            ...
            <div className="body">
              Lembrete enviado por e-mail para{' '}
              <span className="mono">{tx.client.toLowerCase().replace(/\s/g, '')}@cliente.com</span>
            </div>
          </div>
        </div>
        <div className="timeline-item">
          ...
          <div>
            ...
            <div className="body">
              <span className="actor">Cron Ledger</span> · fatura {tx.id} emitida automaticamente.
            </div>
          </div>
        </div>
      </div>
  ...
  <AdminModal
    ...
    footer={
      <>
        ...
        <button className="btn sm">
          <Icon name="download" size={13} /> Gerar e baixar
        </button>
      </>
    }
  >
...
const initialMembers = [
  {
    ...
    status: 'active',
    last: 'agora',
    perms: Object.fromEntries(MODULES.map((m) => [m.id, ['v', 'e', 'a']])),
  },
  {
    id: 2,
    name: 'Ricardo Lopes',
...
const AdminMembers = ({ currentUser }) => {
  ...
  const toggleStatus = (m) =>
    setMembers((prev) =>
      prev.map((p) =>
        p.id === m.id ? { ...p, status: p.status === 'active' ? 'inactive' : 'active' } : p
      )
    );
      ...
      <div className="admin-content">
        {!canManage && (
          ...
          >
            ...
            Você está em modo somente leitura. Apenas Owner e Administradores podem cadastrar ou
            editar membros.
          </div>
        )}
        ...
        <div className="kpi-grid">
          <div className="kpi">
            ...
            <div className="value">{members.filter((m) => m.status === 'active').length}</div>
          </div>
          <div className="kpi">
          ...
          <div className="kpi">
            ...
            <div className="value">{members.filter((m) => m.role === 'Owner').length}</div>
          </div>
          <div className="kpi">
            <div className="label">Convites pendentes</div>
        ...
        <div className="panel">
          ...
          <div className="data-table">
            <div
              className="dt-row header"
              style={{ gridTemplateColumns: '44px 1.4fr 1.4fr 130px 100px 80px 40px' }}
            >
              <span></span>
              <span>Membro</span>
              <span>E-mail</span>
        ...
        <>
          ...
          >
            {[
              ...
              {
                ico: member.status === 'active' ? 'x' : 'check',
                label: member.status === 'active' ? 'Inativar membro' : 'Reativar',
                fn: onToggle,
              },
...
const MemberEditor = ({ member, isNew, onClose, onSave }) => {
  ...
  const setRole = (role) => {
    ...
    if (role === 'Owner' || role === 'Administrador')
      perms = Object.fromEntries(MODULES.map((x) => [x.id, ['v', 'e', 'a']]));
  ...
  return (
    <AdminModal
      title={isNew ? 'Cadastrar membro' : `Editar · ${m.name}`}
      crumbs="/ membros / permissões"
      wide
      onClose={onClose}
      footer={
        <>
    ...
    >
      <div className="fld-group">
              ...
              <div key={mod.id} className="row">
                ...
                <span>
                  <input
                    type="checkbox"
                    className="checkbox"
                    checked={(m.perms[mod.id] || []).includes('v')}
                ...
                <span>
                  <input
                    type="checkbox"
                    className="checkbox"
                    checked={(m.perms[mod.id] || []).includes('e')}
                ...
                <span>
                  <input
                    type="checkbox"
                    className="checkbox"
                    checked={(m.perms[mod.id] || []).includes('a')}
        ...
        >
          ...
          />
          <span>
            O membro receberá um e-mail com link de ativação. O acesso só é concedido após login via
            Google Workspace com o e-mail informado.
          </span>
        </div>
      </div>
    </AdminModal>
  ...
  return (
    <>
      <AdminTopbar
        ...
        crumbs={`/ logs / ${current.id} · supabase`}
        extra={
          <>
            <span
              className="pill"
        ...
        <div className="health-grid">
          {productHealth.map((p) => (
            <div className="health-card" key={p.id}>
              <div className="ll">
                <div className="nm">
                  <span className="pico" style={{ background: p.color }}>
                    {p.iconText}
                  </span>
                  {p.name}
                </div>
                <span className={`status-pill ${p.status}`}>
                  <span className="dt" />
                  {p.status === 'healthy' ? 'saudável' : 'degradado'}
                </span>
              </div>
              <div className="metrics">
                <div className="m">
                  <span className="l">Uptime</span>
                  <span className="v">{p.uptime}</span>
                </div>
                <div className="m">
                  <span className="l">p95</span>
                  <span className="v">{p.p95}</span>
                </div>
                <div className="m">
                  <span className="l">Erros</span>
                  <span
                    className="v"
                    style={{ color: parseFloat(p.err) > 0.5 ? 'var(--pink)' : 'var(--text)' }}
                  >
                    {p.err}
                  </span>
                </div>
          ...
          <div style={{ padding: 18 }}>
            ...
            >
              ...
              <div className="kpi" style={{ padding: 14 }}>
                ...
                <div className="value" style={{ fontSize: 22 }}>
                  {current.events.toLocaleString('pt-BR')}
                </div>
              </div>
            </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 }}>
              ...
              <span className="mono" style={{ fontSize: 11, color: 'var(--text-muted)' }}>
                recebendo via realtime channel · {current.id}.public.*
              </span>
            </div>
            ...
            >
              <div
                className="dt-row header"
                style={{ gridTemplateColumns: '130px 60px 140px 1fr' }}
              >
                <span>Timestamp</span>
                <span>Level</span>
                <span>Operação</span>
              ...
              {events.map((e, i) => (
                <div
                  key={i}
                  className="dt-row"
                  style={{ gridTemplateColumns: '130px 60px 140px 1fr', padding: '9px 0' }}
                >
                  <span style={{ color: 'var(--text-muted)' }}>{e.t}</span>
                  <span>
                    <span
                      className={`level ${e.lvl === 'warn' ? 'warn' : 'info'}`}
                      style={{
                        padding: '2px 6px',
                        borderRadius: 4,
                        fontSize: 9.5,
                        ...
                        fontWeight: 600,
                        background:
                          e.lvl === 'warn' ? 'rgba(231,31,132,0.18)' : 'rgba(127,63,229,0.18)',
                        color: e.lvl === 'warn' ? 'var(--pink)' : 'var(--purple)',
                      }}
                    >
                      {e.lvl.toUpperCase()}
                    </span>
...
const initialNotifs = [
  {
    ...
    desc: 'Disparado quando um lead entra via formulário público.',
    color: '#7f3fe5',
    icon: 'users',
  ...
  {
    id: 2,
    name: 'Ticket crítico aberto',
    desc: 'Severidade `critical` em qualquer produto.',
    color: '#e71f84',
    icon: 'headset',
  ...
  },
  {
    ...
    desc: 'Confirmação de boleto, PIX ou cartão.',
    color: '#66df7a',
    icon: 'check',
    channels: ['in-app'],
    enabled: true,
  },
  {
    ...
    desc: 'Fatura passou da data de vencimento.',
    color: '#e71f84',
    icon: 'bell',
    channels: ['in-app', 'email'],
    enabled: true,
  },
  {
    ...
    desc: 'Promoção de release para o cluster produtivo.',
    color: '#7f3fe5',
    icon: 'bolt',
    channels: ['slack'],
    enabled: true,
  ...
  {
    id: 6,
    name: 'Lead inativo (30+ dias)',
    desc: 'Sem interação registrada há mais de 30 dias.',
    color: '#f59e0b',
    icon: 'bell',
    channels: ['in-app'],
    enabled: false,
  ...
  {
    id: 7,
    name: 'FAQ avaliado negativamente',
    desc: 'Mais de 3 avaliações "não útil" no mesmo artigo.',
    color: '#06b6d4',
    icon: 'x',
    channels: ['in-app'],
    enabled: true,
...
const AdminNotifConfig = () => {
  ...
  const toggle = (n) =>
    setList((prev) => prev.map((p) => (p.id === n.id ? { ...p, enabled: !p.enabled } : p)));

  return (
    <>
      <AdminTopbar
        title="Notificações padrão"
        crumbs="/ operações / notificações"
        extra={
          <button className="btn sm" onClick={() => setAdding(true)}>
            + Nova notificação
          </button>
      ...
      <div className="admin-content">
        <div className="panel">
          <div className="panel-head">
            ...
            <span className="grow" />
            <span className="mono" style={{ fontSize: 11, color: 'var(--text-faint)' }}>
              aplicado a todos os membros do painel
            </span>
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            ...
            {list.map((n) => (
              <div key={n.id} className="notif-row">
                ...
                <button className="menu-btn" onClick={() => setEditing(n)}>
...
const NotifEditor = ({ notif, isNew, onClose, onSave }) => {
  ...
  const toggleCh = (c) =>
    setN((p) => ({
      ...p,
      channels: p.channels.includes(c) ? p.channels.filter((x) => x !== c) : [...p.channels, c],
    }));
  return (
    <AdminModal
      title={isNew ? 'Nova notificação' : `Editar · ${n.name}`}
      crumbs="/ notificações"
      onClose={onClose}
      footer={
        <>
          ...
          <button className="btn sm" onClick={() => onSave(n)}>
            {isNew ? 'Criar' : 'Salvar'} <Icon name="check" size={13} />
          </button>
        </>
      }
    >
      <div className="fld-group">
          ...
          >
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 11.5, color: 'var(--text-muted)', marginTop: 2 }}>
                {n.desc || 'Descrição aparecerá aqui'}
              </div>
            </div>
        ...
        <div className="fld">
          ...
          <textarea
            ...
            placeholder="Quando é disparada?"
          />
        </div>
        <div className="fld-row">
          <div className="fld">
            ...
            <select value={n.icon} onChange={(e) => setN({ ...n, icon: e.target.value })}>
              {['bell', 'users', 'headset', 'check', 'x', 'bolt', 'mail', 'cube', 'bot'].map(
                (i) => (
          ...
          <div className="fld">
            ...
            <div className="color-row">
              {PRESET_COLORS.map((c) => (
                <button
                  key={c}
                  className={`color-swatch ${n.color === c ? 'on' : ''}`}
                  style={{ background: c }}
                  onClick={() => setN({ ...n, color: c })}
                />
              ))}
            </div>
          </div>
        ...
        <div className="fld">
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            {['in-app', 'email', 'slack', 'webhook', 'sms'].map((c) => (
              <button
                key={c}
                className={`filter-chip ${n.channels.includes(c) ? 'on' : ''}`}
                onClick={() => toggleCh(c)}
              >
        ...
        >
          <div>
            ...
            <div style={{ fontSize: 11.5, color: 'var(--text-muted)', marginTop: 2 }}>
              Desativar pausa o envio sem deletar a configuração
            </div>
          </div>
          <button
            className={`toggle ${n.enabled ? 'on' : ''}`}
            onClick={() => setN({ ...n, enabled: !n.enabled })}
          />
        </div>
      </div>
    </AdminModal>
...
const initialCRMCols = [
        ...
        interactions: [
          {
            ...
            who: 'Joana V.',
            kind: 'note',
            body: 'Contato inicial via LinkedIn. RH quer rever programa de incentivos.',
          },
          {
            t: 'ontem · 17:42',
            who: 'Sistema',
            kind: 'email',
            body: 'Lead capturado via formulário público — produto: Pontua.',
          },
        ],
      },
      {
        id: 'n2',
        name: 'Folha Sistemas',
        contact: 'leandro@folha.io',
        tag: 'Avali',
        value: 'R$ 42k/mo',
        responsible: 'Ricardo L.',
        interactions: [
          {
            t: 'hoje · 11:30',
            who: 'Ricardo L.',
            kind: 'call',
            body: 'Call com CTO. Eles querem POC com 800 alunos antes de assinar.',
          },
        ],
      },
      {
        ...
        interactions: [
          {
            ...
            who: 'Joana V.',
            kind: 'call',
            body: 'Demo de 45min. Eles aprovaram tecnicamente. Aguardando aprovação financeira interna.',
          },
        ],
      },
      {
        ...
        interactions: [
          {
            ...
            who: 'Ricardo L.',
            kind: 'note',
            body: 'Proposta enviada. Diretoria revisa na próxima quinta.',
          },
        ],
      },
    ],
  ...
  },
  {
    ...
    cards: [
      ...
      {
        id: 'f2',
        name: 'Bons Negócios SA',
        contact: 'cio@bn.com.br',
        tag: 'Pontua',
        value: 'R$ 28k/mo',
        responsible: 'Joana V.',
        interactions: [],
...
const AdminCRMEnhanced = ({ currentUser }) => {
    ...
    setCols((prev) =>
      prev.map((col) => ({
        ...
        cards: col.cards.map((c) =>
          c.id === cardId ? { ...c, interactions: [interaction, ...c.interactions] } : c
        ),
      }))
    );
    setActive(cardId);
  ...
  const addLead = (lead) => {
    ...
    const card = {
      ...
      interactions: lead.note?.trim()
        ? [{ t: 'agora', who: currentUser.name, kind: 'note', body: lead.note.trim() }]
        : [],
    };
    setCols((prev) =>
      prev.map((c) => (c.id === lead.stage ? { ...c, cards: [card, ...c.cards] } : c))
    );
    setAddingLead(false);
    setActive(id);
  };
  ...
  return (
    <>
      <AdminTopbar
        ...
        crumbs="/ crm / kanban"
        extra={
          <button className="btn sm" onClick={() => setAddingLead(true)}>
            + Novo lead
          </button>
        }
      />
      <div className="admin-content">
        ...
        <div className="kanban">
          {cols.map((col) => (
            <div
              className={`kb-col ${dropCol === col.id ? 'drop-active' : ''}`}
              key={col.id}
              onDragOver={(e) => {
                if (dragCard) {
                  e.preventDefault();
            ...
            >
              ...
              {col.cards.map((c) => (
                <div
                  className={`kb-card ${dragCard === c.id ? 'dragging' : ''}`}
                  key={c.id}
                  draggable
                  onDragStart={(e) => {
                    setDragCard(c.id);
      ...
      {addingLead && window.NewLeadModal && (
        <window.NewLeadModal onClose={() => setAddingLead(false)} onSave={addLead} />
      )}
    </>
  );
};
  ...
  return (
    <aside className="admin-drawer">
      <div className="admin-drawer-head">
        ...
        <h3 style={{ fontSize: 22, letterSpacing: '-0.02em', fontWeight: 500, marginTop: 14 }}>
          {card.name}
        </h3>
        <div className="mono" style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 4 }}>
          {card.contact}
        ...
        >
          <div>
            ...
            <div style={{ fontSize: 16, fontWeight: 500, letterSpacing: '-0.015em', marginTop: 3 }}>
              {card.value}
            </div>
          </div>
          <div>
            ...
            <div className="timeline">
              {card.interactions.map((it, i) => (
                <div key={i} className={`timeline-item ${it.kind}`}>
                  <div className="pip">
                    <Icon
                      name={it.kind === 'call' ? 'headset' : it.kind === 'email' ? 'mail' : 'bot'}
                      size={12}
                    />
                  </div>
                  <div>
                    ...
                    <div className="body">
                      <span className="actor">{it.who}</span> · {it.body}
                    </div>
                  </div>
                </div>
              ))}
      ...
      <form className="admin-drawer-foot" onSubmit={submit}>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'stretch' }}>
          <textarea
            ...
            placeholder={
              kind === 'note'
                ? 'Anotação interna ou observação…'
                : kind === 'call'
                  ? 'Resumo da conversa, próximos passos…'
                  : 'Conteúdo enviado, anexos…'
            }
            style={{
              flex: 1,
              resize: 'none',
