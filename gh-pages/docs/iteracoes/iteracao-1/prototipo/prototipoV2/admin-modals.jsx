...
const BilingualField = ({ label, valuePT, valueEN, onChangePT, onChangeEN, placeholder, multiline = false }) => {
  ...
  return (
    <div className="fld">
      ...
      {tab === "pt" ? (
        <InputEl value={valuePT} onChange={e => onChangePT(e.target.value)} placeholder={placeholder?.pt || placeholder}/>
      ) : (
        <InputEl value={valueEN} onChange={e => onChangeEN(e.target.value)} placeholder={placeholder?.en || placeholder}/>
      )}
    </div>
  );
};
    ...
    <div className="image-drop" onClick={() => inputRef.current?.click()}>
      ...
      {image ? (
        <>
          ...
          <button type="button" className="remove-btn" onClick={e => { e.stopPropagation(); onChange(null); }}>
            <Icon name="x" size={13}/>
          </button>
        </>
      ) : fallback ? (
        <>
          ...
          <button type="button" className="remove-btn" onClick={e => { e.stopPropagation(); inputRef.current?.click(); }}>
            <Icon name="plus" size={13}/>
          </button>
        </>
      ) : (
        <>
          <div className="upload-ico"><Icon name="download" size={16} style={{ transform: "rotate(180deg)" }}/></div>
...
const initialNotifFeed = [
  { id: 1, type: "Novo lead recebido", desc: "Folha Sistemas chegou via formulário público — produto: Avali.", color: "#7f3fe5", icon: "users", t: "agora", unread: true },
  { id: 2, type: "Ticket crítico aberto", desc: "Notifly · #4821 — webhook intermitente para NIVERA Tech.", color: "#e71f84", icon: "headset", t: "14m", unread: true },
  { id: 3, type: "Pagamento recebido", desc: "R$ 24.8k confirmado — NIVERA Tech (Pontua).", color: "#66df7a", icon: "check", t: "2h", unread: true },
  { id: 4, type: "Deploy em produção", desc: "Avali v4.12.0 promovido para o cluster prod-a.", color: "#7f3fe5", icon: "bolt", t: "4h", unread: false },
  { id: 5, type: "FAQ avaliado negativamente", desc: "\"Como configurar fallback SMS\" — 3 votos \"não útil\" hoje.", color: "#06b6d4", icon: "x", t: "1d", unread: false },
  { id: 6, type: "Pagamento em atraso", desc: "Editora Versa — fatura INV-20245 venceu há 2 dias.", color: "#e71f84", icon: "bell", t: "1d", unread: false },
  { id: 7, type: "Novo lead recebido", desc: "Praxis Soluções demonstrou interesse em Pontua.", color: "#7f3fe5", icon: "users", t: "2d", unread: false }
];
    ...
    <>
      ...
      <div className="notif-panel">
        <div className="notif-panel-head">
          <h3>Notificações</h3>
          {unreadCt > 0 && (
            <span style={{ fontFamily: "var(--font-mono)", fontSize: 10, padding: "2px 7px", borderRadius: 100, background: "rgba(231,31,132,0.18)", color: "var(--pink)", letterSpacing: "0.06em" }}>
              {unreadCt} NOVAS
            </span>
          )}
          <span style={{ flex: 1 }}/>
          <button className="clear" onClick={() => setFeed(f => f.map(x => ({ ...x, unread: false })))}>marcar todas</button>
        ...
        <div className="tabs" style={{ padding: "0 14px" }}>
          ...
          <button className={`tab ${tab === "unread" ? "on" : ""}`} onClick={() => setTab("unread")}>
        ...
        <div style={{ flex: 1, overflow: "auto" }}>
          {list.length === 0 ? (
            ...
            <div key={n.id} className={`notif-feed-row ${n.unread ? "unread" : ""}`}
              onClick={() => setFeed(f => f.map(x => x.id === n.id ? { ...x, unread: false } : x))}>
...
// NEW LEAD modal
// ═══════════════════════════════════════════════════
const NewLeadModal = ({ onClose, onSave, products = ["Avali", "Pontua", "Notifly", "Trilho", "Atende", "Ledger"], responsibles = ["Joana V.", "Ricardo L.", "Marina P.", "Tiago A."] }) => {
  const [form, setForm] = useStateMd({
    name: "", contact: "", tag: products[0], value: "", responsible: responsibles[0],
    note: "", stage: "novo"
  });
  ...
  return (
    <AdminModal title="Cadastrar novo lead" crumbs="/ crm / novo" onClose={onClose}
      footer={<>
      ...
      <div className="fld-group">
        <div className="fld-row">
          <div className="fld">
            ...
            <input value={form.name} onChange={e => upd("name", e.target.value)} placeholder="ex. Folha Sistemas" autoFocus/>
          </div>
          <div className="fld">
            <label>Contato (e-mail ou telefone)</label>
            <input value={form.contact} onChange={e => upd("contact", e.target.value)} placeholder="leandro@folha.io"/>
          </div>
        </div>
        <div className="fld-row">
          <div className="fld">
          ...
          <div className="fld">
            ...
            <input value={form.value} onChange={e => upd("value", e.target.value)} placeholder="R$ 24k/mo"/>
          </div>
        </div>
        <div className="fld-row">
          <div className="fld">
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center", padding: 12, background: "var(--bg-soft)", borderRadius: 10, border: "1px solid var(--line)", fontSize: 12, color: "var(--text-muted)" }}>
          <Icon name="users" size={14} style={{ color: "var(--purple)", flexShrink: 0 }}/>
          O lead aparecerá imediatamente no kanban e pode ser arrastado entre as colunas para mudar de estágio.
        </div>
      </div>
    </AdminModal>
  );
...
// ═══════════════════════════════════════════════════
const buildInitialProducts = () => CrxData.products.map((p, i) => ({
  id: p.id,
  name: p.name,
  iconText: p.iconText,
  color: p.color,
  ...
  usedBy: i === 0 ? "NIVERA Tech, Folha Sistemas, Editora Versa" : i === 1 ? "Bons Negócios SA, NIVERA Tech" : i === 2 ? "Aletheia Group, Quantum Lab, Cluster Verde" : "",
  published: i < 4,
  updatedAt: ["há 2 dias", "há 5 dias", "há 1 dia", "há 12 dias", "há 22 dias", "há 30 dias"][i] || "—"
}));
...
const AdminProductsV2 = () => {
  ...
  const filter = (p) => {
    ...
    return p.name.toLowerCase().includes(q) || p.cat.pt.toLowerCase().includes(q) || p.tagline.pt.toLowerCase().includes(q);
  };
  ...
  const togglePublish = (id) => {
    setItems(prev => prev.map(p => p.id === id ? { ...p, published: !p.published, updatedAt: "agora" } : p));
    setMenuOpen(null);
  };
  ...
  return (
    <>
      <AdminTopbar title="Produtos da vitrine" crumbs="/ vitrine / produtos"
        extra={<button className="btn sm" onClick={() => setAdding(true)}>+ Novo produto</button>}/>
      <div className="admin-content">
        <div className="panel" style={{ padding: 14 }}>
          <div className="panel-head" style={{ marginBottom: 4 }}>
            <h3>{items.length} produtos · {items.filter(x => x.published).length} publicados</h3>
            <span className="grow"/>
          ...
          {published.map(p => (
            <div key={p.id}
              ...
              <span className="pico" style={{ background: p.color, backgroundImage: p.image ? `url(${p.image})` : "none", backgroundSize: "cover", backgroundPosition: "center" }}>
                {!p.image && p.iconText}
              </span>
              <div>
              ...
              <span><span className="status-pill paid"><span className="dt"/>publicado</span></span>
              <span className="pdate">edit. {p.updatedAt}</span>
              <span className="mono" style={{ fontSize: 10.5, color: "var(--text-faint)" }}>PT · EN</span>
              <div style={{ position: "relative" }}>
                <button className="menu-btn" onClick={() => setMenuOpen(menuOpen === p.id ? null : p.id)}>
                ...
                {menuOpen === p.id && (
                  <ProductRowMenu p={p}
                    ...
                    onDelete={() => { setDeleting(p); setMenuOpen(null); }}/>
                )}
              </div>
            </div>
          ))}
          ...
          {unpublished.length === 0 && <div className="empty">Todos os produtos estão publicados.</div>}

          {unpublished.map(p => (
            <div key={p.id} className="product-row unpub">
              <span/>
              <span className="pico" style={{ background: p.color, backgroundImage: p.image ? `url(${p.image})` : "none", backgroundSize: "cover" }}>
                {!p.image && p.iconText}
              </span>
              <div>
                <div className="pname">{p.name}</div>
              ...
              <span className="pcat">{p.cat.pt}</span>
              <span><span className="status-pill inactive"><span className="dt"/>rascunho</span></span>
              <span className="pdate">edit. {p.updatedAt}</span>
              <span className="mono" style={{ fontSize: 10.5, color: "var(--text-faint)" }}>PT · EN</span>
              <div style={{ position: "relative" }}>
                <button className="menu-btn" onClick={() => setMenuOpen(menuOpen === p.id ? null : p.id)}>
...
const PRODUCT_COLORS = ["#7f3fe5", "#e71f84", "#66df7a", "#3b82f6", "#f59e0b", "#06b6d4", "#10b981", "#ef4444"];

const ProductEditor = ({ product, isNew, onClose, onSave }) => {
  const [p, setP] = useStateMd(product || {
    id: null,
    name: "",
    iconText: "??",
  ...
  const handleSave = () => {
    const iconText = (p.iconText && p.iconText !== "??") ? p.iconText : p.name.slice(0, 2).toUpperCase();
    onSave({ ...p, iconText });
  };

  return (
    <AdminModal title={isNew ? "Cadastrar novo produto" : `Editar · ${p.name}`}
      crumbs="/ produtos / formulário" wide onClose={onClose}
      footer={<>
        <span className="mono" style={{ fontSize: 10.5, color: "var(--text-faint)", letterSpacing: "0.06em", textTransform: "uppercase", marginRight: "auto" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "200px 1fr", gap: 20 }}>
        ...
        <div className="fld-group">
          ...
          <div className="fld">
            ...
            <input maxLength={3} value={p.iconText} onChange={e => upd("iconText", e.target.value.toUpperCase())} placeholder="AV"/>
          </div>
        </div>
        ...
        <div className="fld-group">
          <div className="fld">
            ...
            <input value={p.name} onChange={e => upd("name", e.target.value)} placeholder="ex. Avali"/>
          </div>

          <BilingualField
            label="Categoria"
            valuePT={p.cat.pt} valueEN={p.cat.en}
          ...
          <BilingualField
            ...
            valuePT={p.tagline.pt} valueEN={p.tagline.en}
            onChangePT={v => updI18n("tagline", "pt", v)}
            onChangeEN={v => updI18n("tagline", "en", v)}
            placeholder={{ pt: "Frase de impacto que descreve o produto", en: "One-line product description" }}/>

          <BilingualField
            label="Descrição completa"
            valuePT={p.description.pt} valueEN={p.description.en}
            onChangePT={v => updI18n("description", "pt", v)}
            onChangeEN={v => updI18n("description", "en", v)}
            multiline
            placeholder={{ pt: "Descreva o produto em 2-3 parágrafos…", en: "Describe the product in 2-3 paragraphs…" }}/>

          <BilingualField
            label="Público-alvo"
            valuePT={p.audience.pt} valueEN={p.audience.en}
            onChangePT={v => updI18n("audience", "pt", v)}
            onChangeEN={v => updI18n("audience", "en", v)}
            placeholder={{ pt: "ex. Redes de ensino, colégios particulares…", en: "e.g. School networks, K-12…" }}/>

          <div className="fld">
            <label>Quem está usando (clientes / logos)</label>
            <input value={p.usedBy} onChange={e => upd("usedBy", e.target.value)} placeholder="NIVERA Tech, Folha Sistemas, Editora Versa"/>
          </div>

          {/* Status block */}
          <div style={{ background: "var(--bg-soft)", border: "1px solid var(--line)", borderRadius: 10, padding: 14 }}>
            <div className="mono" style={{ fontSize: 10, color: "var(--text-faint)", letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: 10 }}>Status de publicação</div>
            <div style={{ display: "flex", gap: 8 }}>
              <button type="button"
                ...
                <div style={{ flex: 1 }}>
                  <div>Publicado</div>
                  <div style={{ fontSize: 11, fontWeight: 400, marginTop: 2, opacity: 0.8 }}>Visível na vitrine pública</div>
                </div>
              </button>
              <button type="button"
                ...
                <div style={{ flex: 1 }}>
                  <div>Rascunho</div>
                  <div style={{ fontSize: 11, fontWeight: 400, marginTop: 2, opacity: 0.8 }}>Salvo, oculto do público</div>
                </div>
              </button>
            </div>
          </div>
...
// FAQ — enhanced CRUD
// ═══════════════════════════════════════════════════
const buildInitialFAQs = () => CrxData.faqs.map((f, i) => ({
  id: `faq-${i + 1}`,
  cat: f.cat,
...
const AdminFaqMgrV2 = () => {
  ...
  const upsert = (f) => {
    setItems(prev => prev.find(p => p.id === f.id) ? prev.map(p => p.id === f.id ? f : p) : [...prev, { ...f, id: f.id || `faq-${Date.now()}`, updatedAt: "agora", views: 0, helpfulPct: 0 }]);
    setEditing(null); setAdding(false);
  };
  const remove = (id) => { setItems(prev => prev.filter(p => p.id !== id)); setDeleting(null); };
  const togglePublish = (id) => {
    setItems(prev => prev.map(p => p.id === id ? { ...p, status: p.status === "published" ? "draft" : "published", updatedAt: "agora" } : p));
    setMenuOpen(null);
  };
  ...
  return (
    <>
      <AdminTopbar title="FAQ · artigos" crumbs="/ vitrine / faq"
        extra={<button className="btn sm" onClick={() => setAdding(true)}>+ Novo artigo</button>}/>
      <div className="admin-content">
        <div className="panel" style={{ padding: 14 }}>
          <div className="panel-head" style={{ marginBottom: 4 }}>
            <h3>{items.length} artigos · {published.length} publicados</h3>
            <span className="grow"/>
            <div className="admin-search" style={{ width: 220 }}>
              <Icon name="search" size={13}/>
              <input placeholder="Buscar pergunta…" value={search} onChange={e => setSearch(e.target.value)}/>
            </div>
          </div>

          <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginTop: 10 }}>
            <button className={`filter-chip ${catFilter === "all" ? "on" : ""}`} onClick={() => setCatFilter("all")}>
              todas <span style={{ fontFamily: "var(--font-mono)", fontSize: 10, opacity: 0.7 }}>{items.length}</span>
            </button>
            {FAQ_CATEGORIES.map(c => {
              const ct = items.filter(i => i.cat === c).length;
              return (
                <button key={c} className={`filter-chip ${catFilter === c ? "on" : ""}`} onClick={() => setCatFilter(c)}>
                  {c} <span style={{ fontFamily: "var(--font-mono)", fontSize: 10, opacity: 0.7 }}>{ct}</span>
                </button>
              );
            })}
          </div>
          ...
          {published.length === 0 && <div className="empty">Nenhum artigo publicado nesta categoria.</div>}
          ...
          {published.map(item => (
            <FaqRow key={item.id} item={item}
              ...
              onEdit={() => { setEditing(item); setMenuOpen(null); }}
              onTogglePublish={() => togglePublish(item.id)}
              onDelete={() => { setDeleting(item); setMenuOpen(null); }}/>
          ))}
      ...
      {(adding || editing) && (
        <FaqEditor faq={editing} isNew={adding}
          onClose={() => { setEditing(null); setAdding(false); }}
          onSave={upsert}/>
      )}
...
const FaqRow = ({ item, menuOpen, onMenuToggle, onMenuClose, onEdit, onTogglePublish, onDelete }) => (
  <div className={`faq-row ${item.status === "draft" ? "draft" : ""}`}>
    <span className="qmark">?</span>
    <span className="mono" style={{ fontSize: 10, padding: "3px 7px", background: "var(--bg-soft)", borderRadius: 4, textTransform: "uppercase", letterSpacing: "0.06em", textAlign: "center", color: "var(--text-muted)" }}>
      {item.cat}
    </span>
    ...
    <span>
      <span className={`status-pill ${item.status === "published" ? "paid" : "inactive"}`}>
        <span className="dt"/>{item.status === "published" ? "publicado" : "rascunho"}
      </span>
    </span>
    <span className="mono" style={{ fontSize: 11, color: "var(--text-muted)", textAlign: "right" }}>{item.helpfulPct}% útil</span>
    <span className="mono" style={{ fontSize: 10.5, color: "var(--text-faint)", textAlign: "right" }}>{item.views} views</span>
    <div style={{ position: "relative" }}>
      <button className="menu-btn" onClick={onMenuToggle}><Icon name="moreV" size={14}/></button>
      {menuOpen && (
        <>
          <div style={{ position: "fixed", inset: 0, zIndex: 30 }} onClick={onMenuClose}/>
          <div className="row-menu-pop">
            <button onClick={onEdit}><Icon name="settings" size={13}/> Editar artigo</button>
            <button onClick={onTogglePublish}>
...
const FaqEditor = ({ faq, isNew, onClose, onSave }) => {
  ...
  return (
    <AdminModal title={isNew ? "Novo artigo de FAQ" : `Editar · ${f.q.pt || "Artigo"}`}
      crumbs="/ faq / formulário" wide onClose={onClose}
      footer={<>
        <span className="mono" style={{ fontSize: 10.5, color: "var(--text-faint)", letterSpacing: "0.06em", textTransform: "uppercase", marginRight: "auto" }}>
      ...
      <div className="fld-group">
        ...
        <BilingualField
          label="Pergunta"
          valuePT={f.q.pt} valueEN={f.q.en}
          onChangePT={v => updI18n("q", "pt", v)}
          onChangeEN={v => updI18n("q", "en", v)}
          placeholder={{ pt: "Como funciona a contratação?", en: "How does the subscription work?" }}/>

        <BilingualField
          label="Resposta"
          valuePT={f.a.pt} valueEN={f.a.en}
        ...
        <div style={{ background: "var(--accent-soft)", border: "1px solid var(--accent-line)", borderRadius: 10, padding: 12, fontSize: 12.5, color: "var(--text)", display: "flex", gap: 10, alignItems: "flex-start" }}>
          <Icon name="sparkle" size={14} style={{ flexShrink: 0, marginTop: 2, color: "var(--purple)" }}/>
          <div>
            <div style={{ fontWeight: 500, marginBottom: 2 }}>Dica</div>
            <div style={{ color: "var(--text-muted)" }}>Mantenha respostas curtas e objetivas. Use markdown leve (negrito, listas) — está habilitado.</div>
          </div>
        </div>
      </div>
    </AdminModal>
  ...
  <div className="admin-overlay" onClick={onClose}>
    <div className="admin-modal" style={{ maxWidth: 460 }} onClick={e => e.stopPropagation()}>
      <div className="admin-modal-body danger-modal">
        <div className="danger-icon"><Icon name="x" size={24}/></div>
        <h4>{title}</h4>
        <p>{warning}</p>
