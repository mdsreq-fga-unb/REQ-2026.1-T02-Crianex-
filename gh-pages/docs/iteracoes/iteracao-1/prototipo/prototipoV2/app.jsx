...
const Toolbar = ({ surface, setSurface, device, setDevice, page, setPage, lang, setLang }) => {
  return (
    <div className="toolbar">
      ...
      <span className="crumb">/ {surface === "vitrine" ? "vitrine.crianex.com" : "admin.crianex.com (interno)"}</span>

      <div style={{ flex: 1 }}/>

      {/* Surface switch */}
      <div className="seg">
        <button className={surface === "vitrine" ? "on" : ""} onClick={() => { setSurface("vitrine"); setPage({ name: "home" }); }}>
      ...
      {/* Device switch */}
      <div className="seg">
        <button className={device === "desktop" ? "on" : ""} onClick={() => setDevice("desktop")} title="Desktop · 1280">
          <Icon name="desktop" size={12}/>
        </button>
        <button className={device === "tablet" ? "on" : ""} onClick={() => setDevice("tablet")} title="Tablet · 820">
          <Icon name="tablet" size={12}/>
        </button>
        <button className={device === "mobile" ? "on" : ""} onClick={() => setDevice("mobile")} title="Mobile · 390">
      ...
      {surface === "vitrine" && (
        <div className="seg" style={{ fontFamily: "var(--font-mono)" }}>
          <button className={lang === "pt" ? "on" : ""} onClick={() => setLang("pt")} style={{ letterSpacing: "0.06em" }}>PT 🇧🇷</button>
          <button className={lang === "en" ? "on" : ""} onClick={() => setLang("en")} style={{ letterSpacing: "0.06em" }}>EN 🇺🇸</button>
        </div>
      )}
  ...
  return (
    <>
      ...
      <main>
        ...
        {page.name === "product" && <ProductPage lang={lang} productId={page.id} setPage={setPage}/>}
        {page.name === "about" && <AboutPage lang={lang}/>}
        {page.name === "contact" && <ContactPage lang={lang}/>}
        {page.name === "faq" && <FaqPage lang={lang}/>}
      </main>
      ...
      <div className="stage">
        <div className={`viewport ${device}`}>
          ...
          <div className="viewport-inner" key={`${surface}-${device}`}>
            {surface === "vitrine"
              ? <VitrineApp lang={lang} setLang={setLang}/>
              : <AdminApp/>}
          </div>
        </div>
      </div>
