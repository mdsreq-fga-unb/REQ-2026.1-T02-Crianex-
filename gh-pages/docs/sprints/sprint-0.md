# Sprint 0 — Unidade 1

**Período:** 09/04/2026 – 14/04/2026
**Status:** ✅ Concluída
**Entrega:** Documento de Visão — GitHub Pages

---

## Objetivo

Sprint inicial de alinhamento com o cliente e fundação do projeto. O foco foi a produção do **Documento de Visão**, reconhecimento de problema do cliente e a configuração da infraestrutura de documentação (GitHub Pages via MkDocs), com entrega final em **14/04/2026**.

---

## Entregas

| # | Entrega | Responsável | Status |
|---|---------|-------------|--------|
| 1 | Seções 2.4 a 2.7 do Documento de Visão | Hugo e Felipe | ✅ |
| 2 | Seções 3.1 a 3.3 do Documento de Visão | Camille e Leonardo | ✅ |
| 3 | Seções 4.1, 4.2 e Tabela de Engenharia de Requisitos | Heitor e Techlead| ✅ |
| 4 | Lições Aprendidas (10.1), Cronograma e GitHub Pages | Techlead| ✅ |
| 5 | Cenário Atual, Solução Proposta e Correções gerais | Toda a Equipe | ✅ |

---

## Reunião de Alinhamento — Ata 0

A reunião de kickoff desta sprint ocorreu em **09/04/2026** via Discord, com a participação de toda a equipe e o Product Owner.

:material-file-document: [Ver Ata 0 — 09/04/2026](../atas/2026-04-09.md)

---

## Gravação da Reunião

!!! info "Adicionar vídeo"
    Coloque o arquivo `.mp4` da reunião na pasta `docs/sprints/videos/` e atualize o nome abaixo.

<div style="width: 100%; max-width: 900px; margin: 1.5rem 0; border-radius: 8px; overflow: hidden; box-shadow: 0 4px 16px rgba(0,0,0,0.15);">
  <video controls style="width: 100%; display: block;">
    <source src="../videos/reuniao-sprint-0.mp4" type="video/mp4">
    <p>Seu navegador não suporta reprodução de vídeo.
    <a href="../videos/reuniao-sprint-0.mp4">Baixar o vídeo</a>.</p>
  </video>
</div>

---

## Cronograma da Sprint

```mermaid
gantt
    title Sprint 0 — Unidade 1
    dateFormat  YYYY-MM-DD
    section Planejamento
    Reunião de alinhamento     :done, 2026-04-09, 1d
    section Desenvolvimento
    Seções 2.4–2.7             :done, 2026-04-10, 3d
    Seções 3.1–3.3             :done, 2026-04-10, 3d
    Seções 4.1–4.2             :done, 2026-04-10, 3d
    Lições Aprendidas          :done, 2026-04-10, 3d
    section Entrega
    Deploy GitHub Pages        :done, 2026-04-13, 1d
    Entrega Final              :milestone, 2026-04-14, 0d
```
